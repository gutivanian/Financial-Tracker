...
-- Clean version - Ready for production
-- ========================================
...
CREATE TABLE investment_instruments (
    ...
    asset_type VARCHAR(50) NOT NULL, -- 'stocks_id', 'stocks_us', 'crypto', 'bonds', 'commodities', 'etf'
    market VARCHAR(100), -- 'IDX', 'NASDAQ', 'NYSE', 'Crypto', etc
    currency VARCHAR(10) DEFAULT 'IDR',
    country VARCHAR(50),
    description TEXT,
    logo_url TEXT,
    price_source VARCHAR(50) NOT NULL, -- 'coingecko', 'yahoo_finance', 'alpha_vantage', 'finnhub'
    price_mapping VARCHAR(100) NOT NULL, -- Ticker/ID used in the price source API
    last_price DECIMAL(20, 8), -- Last successfully fetched price in original currency
    last_price_idr DECIMAL(20, 2), -- Last successfully fetched price in IDR
    last_updated TIMESTAMP, -- Timestamp of last successful price fetch
    price_fetch_error TEXT, -- Last error message if price fetch failed
    is_active BOOLEAN DEFAULT true,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    UNIQUE(symbol, market),
    CONSTRAINT check_price_source_not_manual CHECK (price_source != 'manual')
);
...
-- Investments table (simplified - all details from investment_instruments)
...
COMMENT ON TABLE investment_instruments IS 'Master data for all tradeable investment instruments (stocks, crypto, ETFs, commodities)';
COMMENT ON TABLE investments IS 'User portfolio holdings - references investment_instruments for instrument details';
COMMENT ON COLUMN investment_instruments.price_source IS 'API source for price fetching: coingecko, yahoo_finance, alpha_vantage, finnhub (manual not allowed)';
...
COMMENT ON COLUMN investments.instrument_id IS 'References investment_instruments - all instrument details stored there';
...
('demo@finance.com', 'Demo User', NULL);
...
-- Sample investment instruments master data (NO MANUAL SOURCES)
INSERT INTO investment_instruments (name, symbol, asset_type, market, currency, country, price_source, price_mapping) VALUES
-- Indonesian Stocks
('PT Bank Central Asia Tbk', 'BBCA', 'stocks_id', 'IDX', 'IDR', 'Indonesia', 'yahoo_finance', 'BBCA.JK'),
('PT Bank Mandiri Tbk', 'BMRI', 'stocks_id', 'IDX', 'IDR', 'Indonesia', 'yahoo_finance', 'BMRI.JK'),
('PT Bank Rakyat Indonesia Tbk', 'BBRI', 'stocks_id', 'IDX', 'IDR', 'Indonesia', 'yahoo_finance', 'BBRI.JK'),
('PT Bank Negara Indonesia Tbk', 'BBNI', 'stocks_id', 'IDX', 'IDR', 'Indonesia', 'yahoo_finance', 'BBNI.JK'),
('PT Telkom Indonesia Tbk', 'TLKM', 'stocks_id', 'IDX', 'IDR', 'Indonesia', 'yahoo_finance', 'TLKM.JK'),
('PT Unilever Indonesia Tbk', 'UNVR', 'stocks_id', 'IDX', 'IDR', 'Indonesia', 'yahoo_finance', 'UNVR.JK'),
('PT Astra International Tbk', 'ASII', 'stocks_id', 'IDX', 'IDR', 'Indonesia', 'yahoo_finance', 'ASII.JK'),
('PT Gudang Garam Tbk', 'GGRM', 'stocks_id', 'IDX', 'IDR', 'Indonesia', 'yahoo_finance', 'GGRM.JK'),

-- US Stocks
('Apple Inc', 'AAPL', 'stocks_us', 'NASDAQ', 'USD', 'United States', 'yahoo_finance', 'AAPL'),
('Microsoft Corporation', 'MSFT', 'stocks_us', 'NASDAQ', 'USD', 'United States', 'yahoo_finance', 'MSFT'),
('NVIDIA Corporation', 'NVDA', 'stocks_us', 'NASDAQ', 'USD', 'United States', 'yahoo_finance', 'NVDA'),
('Alphabet Inc Class A', 'GOOGL', 'stocks_us', 'NASDAQ', 'USD', 'United States', 'yahoo_finance', 'GOOGL'),
('Amazon.com Inc', 'AMZN', 'stocks_us', 'NASDAQ', 'USD', 'United States', 'yahoo_finance', 'AMZN'),
('Tesla Inc', 'TSLA', 'stocks_us', 'NASDAQ', 'USD', 'United States', 'yahoo_finance', 'TSLA'),
('Meta Platforms Inc', 'META', 'stocks_us', 'NASDAQ', 'USD', 'United States', 'yahoo_finance', 'META'),
('Netflix Inc', 'NFLX', 'stocks_us', 'NASDAQ', 'USD', 'United States', 'yahoo_finance', 'NFLX'),

-- Cryptocurrency
('Bitcoin', 'BTC', 'crypto', 'Crypto', 'USD', 'Global', 'coingecko', 'bitcoin'),
('Ethereum', 'ETH', 'crypto', 'Crypto', 'USD', 'Global', 'coingecko', 'ethereum'),
('Binance Coin', 'BNB', 'crypto', 'Crypto', 'USD', 'Global', 'coingecko', 'binancecoin'),
('Ripple', 'XRP', 'crypto', 'Crypto', 'USD', 'Global', 'coingecko', 'ripple'),
('Cardano', 'ADA', 'crypto', 'Crypto', 'USD', 'Global', 'coingecko', 'cardano'),
('Solana', 'SOL', 'crypto', 'Crypto', 'USD', 'Global', 'coingecko', 'solana'),
('Polkadot', 'DOT', 'crypto', 'Crypto', 'USD', 'Global', 'coingecko', 'polkadot'),

-- Commodities
('Gold', 'GOLD', 'commodities', 'Commodity', 'USD', 'Global', 'yahoo_finance', 'GC=F'),
('Silver', 'SILVER', 'commodities', 'Commodity', 'USD', 'Global', 'yahoo_finance', 'SI=F'),
('Crude Oil', 'OIL', 'commodities', 'Commodity', 'USD', 'Global', 'yahoo_finance', 'CL=F'),

-- ETFs
('SPDR S&P 500 ETF Trust', 'SPY', 'etf', 'NYSE', 'USD', 'United States', 'yahoo_finance', 'SPY'),
('Vanguard Total Stock Market ETF', 'VTI', 'etf', 'NYSE', 'USD', 'United States', 'yahoo_finance', 'VTI'),
('Invesco QQQ Trust', 'QQQ', 'etf', 'NASDAQ', 'USD', 'United States', 'yahoo_finance', 'QQQ');
...
(1, 1, 17, 'expense', 1500000, CURRENT_DATE - INTERVAL '24 days', 'Beli saham & crypto', 'Ajaib'),
...
-- Sample investments (using instrument_id from investment_instruments)
INSERT INTO investments (user_id, instrument_id, purchase_date, purchase_price, quantity, platform, notes) VALUES
(1, 1, '2024-01-15', 9500, 100, 'Ajaib', 'Long term investment'),
(1, 2, '2024-02-20', 6200, 150, 'Ajaib', 'Banking sector'),
(1, 3, '2024-03-10', 5800, 200, 'Ajaib', 'Banking sector'),
(1, 9, '2023-11-05', 180.50, 10, 'Interactive Brokers', 'Tech stock'),
(1, 10, '2024-01-20', 380.00, 5, 'Interactive Brokers', 'Tech stock'),
(1, 17, '2023-08-15', 28500, 0.05, 'Indodax', 'Crypto investment'),
(1, 18, '2023-09-20', 1850, 0.5, 'Indodax', 'Crypto investment'),
(1, 25, '2024-03-01', 1950, 20, 'Yahoo Finance', 'Gold commodity');
...
INSERT INTO debt_payments (debt_id, amount, payment_date, principal_amount, interest_amount, notes) VALUES
(1, 500000, CURRENT_DATE - INTERVAL '15 days', 425000, 75000, 'Monthly payment'),
(2, 600000, CURRENT_DATE - INTERVAL '10 days', 475000, 125000, 'Monthly payment');
...
BEGIN
    ...
    RAISE NOTICE 'Sample user created: demo@finance.com';
    RAISE NOTICE 'Investment instruments: % rows', (SELECT COUNT(*) FROM investment_instruments);
    RAISE NOTICE 'Sample investments: % rows', (SELECT COUNT(*) FROM investments);
    RAISE NOTICE 'Sample transactions: % rows', (SELECT COUNT(*) FROM transactions);
    RAISE NOTICE '=========================================';
    RAISE NOTICE 'Key Features:';
    RAISE NOTICE '- NO manual price sources allowed';
    RAISE NOTICE '- Investment instruments normalized';
    RAISE NOTICE '- Automatic price fetching support';
    RAISE NOTICE '- Clean relational structure';
    RAISE NOTICE '=========================================';
END $$;